...
class LSD_PT_Dimensions_And_Precision_Transforms:
    ...
    def draw(layout: bpy.types.UILayout, context: bpy.types.Context) -> None:
        ...
        if is_expanded:
            ...
            anchor_box_outer = col_main.box()
            header_row = anchor_box_outer.row()
            icon = 'TRIA_DOWN' if scene.lsd_show_subpanel_parametric_anchors else 'TRIA_RIGHT'
            header_row.prop(scene, "lsd_show_subpanel_parametric_anchors", text="", icon=icon, emboss=False)
            header_row.label(text="Parametric Anchors", icon='HOOK')
            if scene.lsd_show_subpanel_parametric_anchors:
                anchor_box = anchor_box_outer.column()
            ...
            dim_toolkit_box_outer = col_main.box()
            header_row = dim_toolkit_box_outer.row()
            icon = 'TRIA_DOWN' if scene.lsd_show_subpanel_dimension_generator else 'TRIA_RIGHT'
            header_row.prop(scene, "lsd_show_subpanel_dimension_generator", text="", icon=icon, emboss=False)
            header_row.label(text="Dimension Generator & Preferences", icon='DRIVER_DISTANCE')
            if scene.lsd_show_subpanel_dimension_generator:
                dim_toolkit_box = dim_toolkit_box_outer.column()
                ...
                col.separator()
            # --- Accurate Scale (New) ---
            scale_box_outer = col_main.box()
            header_row = scale_box_outer.row()
            icon = 'TRIA_DOWN' if scene.lsd_show_subpanel_accurate_scaling else 'TRIA_RIGHT'
            header_row.prop(scene, "lsd_show_subpanel_accurate_scaling", text="", icon=icon, emboss=False)
            header_row.label(text="Accurate Scaling", icon='CON_FOLLOWPATH')
            if scene.lsd_show_subpanel_accurate_scaling:
                scale_box = scale_box_outer.column()
