...
class FCD_UL_ExportList(bpy.types.UIList):
...
class FCD_PT_Import_Export_System:
    """
    AI Editor Note:
    ...
    def poll(cls, context: bpy.types.Context) -> bool:
        ...
        return context.scene.fcd_panel_enabled_export
    ...
    def draw(layout: bpy.types.UILayout, context: bpy.types.Context) -> None:
        ...
        box, is_expanded = ui_common.draw_panel_header(
            ...
            "fcd_show_panel_export", 
            "fcd_panel_enabled_export"
        )
        ...
        if is_expanded:
            ...
            export_sys_box.operator("fcd.generate_ros2_workspace", text="Generate ROS 2 Workspace", icon='FILE_NEW')
            ...
            row = col.row()
            row.template_list("FCD_UL_ExportList", "", scene, "fcd_export_list", scene, "fcd_export_list_index", rows=3)
            col_ops = row.column(align=True)
            col_ops.operator("fcd.export_list_add", text="", icon='ADD')
            col_ops.operator("fcd.export_list_remove", text="", icon='REMOVE')
            ...
            flow.prop(scene, "fcd_export_check_meshes")
            flow.prop(scene, "fcd_export_check_textures")
            flow.prop(scene, "fcd_export_check_config")
            flow.prop(scene, "fcd_export_check_launch")
            
            col.separator()
            col.prop(scene, "fcd_export_mesh_format")
            ...
            row.scale_y = 1.5
            row.operator("fcd.export_general", text="Export Package")
            
            # Tools
            export_sys_box.separator()
            export_sys_box.operator("fcd.export_gazebo_world", text="Export Gazebo World", icon='WORLD')
            
            # --- 4. Quick Export Selected ---
            export_sys_box.separator()
            export_sys_box.label(text="4. Quick Export (Selected)", icon='EXPORT')
            row = export_sys_box.row(align=True)
            row.prop(scene, "fcd_quick_export_format", text="")
            row.operator("fcd.export_selected", text="Export Mesh", icon='SCENE_DATA')


def register():
    for cls in [FCD_UL_ExportList, FCD_PT_Import_Export_System]:
        if hasattr(cls, 'bl_rna'):
            bpy.utils.register_class(cls)

def unregister():
    for cls in reversed([FCD_UL_ExportList, FCD_PT_Import_Export_System]):
        if hasattr(cls, 'bl_rna'):
