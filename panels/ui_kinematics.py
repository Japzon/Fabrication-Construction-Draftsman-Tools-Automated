...
class FCD_PT_Kinematics_Setup:
    ...
    def poll(context: bpy.types.Context) -> bool:
        return context.scene.fcd_panel_enabled_kinematics
    ...
    def draw(layout: bpy.types.UILayout, context: bpy.types.Context) -> None:
        ...
        box, is_expanded = ui_common.draw_panel_header(
            layout, context, 
            "Kinematics Setup", 
            "fcd_show_panel_kinematics", 
            "fcd_panel_enabled_kinematics"
        )
        ...
        if is_expanded:
            ...
            row.operator("fcd.snap_cursor_to_active", text="To Origin")
            ...
            if context.active_object:
                ...
                cursor_box.prop(scene, "fcd_cursor_local_pos", text="Local")
            else:
            ...
            cursor_box.operator("fcd.set_origin_to_cursor", icon='OBJECT_ORIGIN')
            ...
            row.operator("fcd.merge_armatures", icon='LINKED', text="Merge Armatures")

            # --- Section: Bone Tools ---
            bone_tools_box = box.box()
            bone_tools_box.label(text="Bone Tools", icon='BONE_DATA')
            col = bone_tools_box.column(align=True)
            col.prop(scene, "fcd_bone_mode", text="")
            col.prop(scene, "fcd_bone_axis", text="Align Axis")
            col.operator("fcd.add_bone", icon='BONE_DATA', text="Set / Align Bones")
            col.operator("fcd.purge_bones", icon='TRASH', text="Purge Selected Bones")
            col.operator("fcd.parent_to_active", icon='LINKED', text="Parent Selected to Active")

            if context.mode == 'OBJECT':
                bone_tools_box.operator("fcd.enter_pose_mode", icon='POSE_HLT')
            else:
                bone_tools_box.operator("fcd.enter_object_mode", icon='OBJECT_DATAMODE')
            ...
            if show_editor:
                ...
                if hasattr(scene, "fcd_placement_mode") and scene.fcd_placement_mode:
                    placement_box.operator("fcd.toggle_placement", text="Stop Joint Placement Mode", icon='CANCEL')
                else:
                    placement_box.operator("fcd.toggle_placement", text="Start Joint Placement Mode", icon='POSE_HLT')
                
                placement_box.operator("fcd.apply_rest_pose", icon='ARMATURE_DATA', text="Apply as Rest Pose")
                ...
                if active_bone and active_obj and active_obj.type == 'ARMATURE':
                    props = active_bone.fcd_pg_kinematic_props
                    ...
                    if len(props.mimic_drivers) > 0:
                        ...
                        for i, m in enumerate(props.mimic_drivers):
                            ...
                            op = r.operator("fcd.remove_mimic", text="", icon='X')
                            op.index = i
                    ...
                    if active_obj.pose:
                        ...
                        op_t = row_t.operator("fcd.pick_bone", text="", icon='EYEDROPPER')
                        op_t.mode = 0
                        row_ref = add_box.row(align=True)
                        row_ref.prop_search(props, "ratio_ref_bone", active_obj.pose, "bones", text="Ref Bone")
                        op_r = row_ref.operator("fcd.pick_bone", text="", icon='EYEDROPPER')
                        op_r.mode = 1
                    else:
                    ...
                    row_r.operator("fcd.calculate_ratio", text="", icon='DRIVER_DISTANCE')
                    add_box.operator("fcd.add_mimic", icon='ADD', text="Add / Update Driver")
                else:
...
def register():
    for cls in [FCD_PT_Kinematics_Setup]:
        if hasattr(cls, 'bl_rna'):
            bpy.utils.register_class(cls)

def unregister():
    for cls in reversed([FCD_PT_Kinematics_Setup]):
        if hasattr(cls, 'bl_rna'):
