...
class FCD_PT_Transmission:
    ...
    def poll(cls, context: bpy.types.Context) -> bool:
        ...
        return context.scene.fcd_panel_enabled_transmission
    ...
    def draw(layout: bpy.types.UILayout, context: bpy.types.Context) -> None:
        ...
        box, is_expanded = ui_common.draw_panel_header(
            layout, context, 
            "Transmission", 
            "fcd_show_panel_transmission", 
            "fcd_panel_enabled_transmission"
        )
        
        if is_expanded:
            pb = context.active_pose_bone
            if pb:
                props = pb.fcd_pg_kinematic_props
...
def register():
    for cls in [FCD_PT_Transmission]:
        if hasattr(cls, 'bl_rna'):
            bpy.utils.register_class(cls)

def unregister():
    for cls in reversed([FCD_PT_Transmission]):
        if hasattr(cls, 'bl_rna'):
